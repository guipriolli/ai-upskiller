...
[ -d "$HOME/.sdkman" ] && OPTIONAL_BINDS="$OPTIONAL_BINDS --ro-bind $HOME/.sdkman $HOME/.sdkman"
[ -d "$HOME/.local" ] && OPTIONAL_BINDS="$OPTIONAL_BINDS --ro-bind $HOME/.local $HOME/.local"

# SSH agent socket - only bind if SSH_AUTH_SOCK is set and exists
SSH_BINDS=""
SSH_ENV=""
if [ -n "$SSH_AUTH_SOCK" ] && [ -S "$SSH_AUTH_SOCK" ]; then
  SSH_BINDS="--ro-bind $(dirname "$SSH_AUTH_SOCK") $(dirname "$SSH_AUTH_SOCK") --ro-bind $SSH_AUTH_SOCK $SSH_AUTH_SOCK"
